    ...
    try {
        ...
        for(let j=0;j<data3.length;j++){
            ...
            const taskcreation = `insert into TASK(userEmpCode,taskDescription,startDate,endDate,assignedBy,assignedDate,companyId,status)
                               values('${obj.userEmpCode}','${obj.taskDescription}','${objstartDate}','${objendDate}','${req.params.Empcode}','${date1}','${req.params.compId}','${obj.status}') `
...
exports.taskstatusUpdate = async (req, res) => {
 ...
 console.log(req.body)
    try {
        let update = `update task set status='Done' where taskId='${req.params.taskId}'`
...
exports.gettasks = async (req, res) => {
    ...
    try {
        ...
        let GetTask = `select * from task where (userempcode='${req.params.EmpCode}' and status='Todo' or status = 'Inprogress')`;
